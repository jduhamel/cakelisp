...
This script first builds cakelisp explicitly, then uses the ~cakelisp_bootstrap_v3~ executable to build Cakelisp using Cakelisp. Subsequent executions of ~Build.sh~ will also build using Cakelisp, which means all the caching features will be in effect (making builds much faster).
