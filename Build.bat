...
if not exist "bin\cakelisp_bootstrap_v3.exe" (
  goto manualBuild
) else (
  goto bootstrapBuild
)
...
CL.exe src/Tokenizer.cpp ^
 ...
 /Fe"bin\cakelisp_bootstrap_v3" /Zi /Fd"bin\cakelisp_bootstrap_v3.pdb" /DEBUG:FASTLINK
 echo %ERRORLEVEL%
...
:bootstrapBuild
"bin\cakelisp_bootstrap_v3.exe" Bootstrap_MSVC.cake
@if %ERRORLEVEL% == 0 (
